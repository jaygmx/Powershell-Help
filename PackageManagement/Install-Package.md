...
\`Install-Package\` uses parameters to specify the packages Name and Source .
The Credential parameter uses a domain user account with permissions to install packages.
The command prompts you for the user account password.
...
\`Find-Package\` uses the Name and Source parameters to locate a package.
The object is sent down the pipeline and \`Install-Package\` installs the package on the local computer.
...
\`Install-Package\` uses the Name and Source parameters to find a package.
The MinimumVersion and MaximumVersion parameters specify a range of software versions.
The highest version in the range is installed.
...
### -AllVersions
\`Install-Package\` installs all available versions of the package.
By default, only the newest version is installed.
...
Type a user name, such as User01 , Domain01\User01 , or enter a PSCredential object, generated by the \`Get-Credential\` cmdlet.
If you type a user name, you're prompted for a password.
...
### -DscResource
Specifies one or more Desired State Configuration (DSC) resources that are searched by \`Install-Package\`.
...
Overrides restrictions that prevent \`Install-Package\` from succeeding, with the exception of security.
...
### -Includes
Specifies whether \`Install-Package\` should find all package types.
...
### -MaximumVersion
Specifies the maximum allowed package version that you want to install.
If you don't specify this parameter, \`Install-Package\` installs the package's newest version.
...
### -MinimumVersion
Specifies the minimum allowed package version that you want to install.
If you don't add this parameter, \`Install-Package\` installs the package's newest version that satisfies any version specified by the MaximumVersion parameter.
...
### -Scope
Specifies the scope for which to install the package.
The acceptable values for this parameter are as follows:
...
### -WhatIf
Shows what would happen if \`Install-Package\` cmdlet is run.
...
For example, \`Install-Package\` has the PowerShellGet parameter set that includes \`-NoPathUpdate\`, \`AllowClobber\`, and \`SkipPublisherCheck\`.
