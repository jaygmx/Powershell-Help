...
## DESCRIPTION
\`Find-Package\` finds software packages that are available in package sources.
...
\`Find-Package\` uses the Name parameter to specify the package name NuGet.Core .
The Source parameter specifies to search for the package in MyNuGet .
...
\`Find-Package\` uses the Name parameter to specify the package NuGet.Core .
The ProviderName parameter specifies to search for the package in MyNuGet .
AllVersions specifies that all available versions are returned.
...
\`Find-Package\` uses the Name parameter to specify the package name NuGet.Core .
The ProviderName parameter specifies to search for the package in NuGet .
RequiredVersion specifies that only version 2.9.0 is returned.
...
\`Find-Package\` uses the Name parameter to specify the package name NuGet.Core .
The ProviderName parameter specifies to search for the package in NuGet .
MinimumVersion specifies the lowest version 2.7.0 .
MaximumVersion specifies the highest version 2.9.0 .
...
### -FilterOnTag
Specifies the tag that filters the results.
Results that don't contain the specified tag are excluded.
...
Use \`Get-PackageSource\` to get a list of available package sources.
A file system directory can be used as a source for download packages.
