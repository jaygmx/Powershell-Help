...
## DESCRIPTION
The \`Get-PSReadLineOption\` cmdlet returns the current state of the settings that can be configured by using the \`Set-PSReadLineOption\` cmdlet.
You can use the returned object to change PSReadLine options.
...
Changing the property values of this object updates the settings in PSReadLine directly without invoking \`Set-PSReadLineOption\`.
