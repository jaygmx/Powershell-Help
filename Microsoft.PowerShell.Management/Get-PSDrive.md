...
drives) and the HKLM: and HKCU: drives that are exposed by the Windows PowerShell Registry   provider.
...
Like net use , it returns only the persistent G: drive created by \`New-PSDrive\`.
