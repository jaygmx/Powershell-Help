...
The order of the values entered isn't important.
...
When using this parameter, the cmdlet creates a temporary session to the specified computer using the WsMan protocol.
Otherwise, the cmdlet performs the operation on the local computer using Component Object Model (COM).
...
When you specify this parameter without specifying the ComputerName parameter, or when you specify a CIM session created using DCOM protocol, you get an error.
The DCOM protocol does not support the ResourceURI parameter.
