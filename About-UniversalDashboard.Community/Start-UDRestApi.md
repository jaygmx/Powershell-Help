...
```
Start-UDRestApi [-AutoReload] [-Certificate <X509Certificate2>] [-CertificateFile <String>]
 [-CertificateFilePassword <SecureString>] [-DisableTelemetry] [-Endpoint <Endpoint[]>]
 [-EndpointInitialization <InitialSessionState>] [-Force] [-ListenAddress <IPAddress>] [-Name <String>]
 [-Port <Int32>] [-PublishedFolder <PublishedFolder[]>] [-Wait] [<CommonParameters>]
```

## DESCRIPTION
Starts a REST API server.
This server consists only of REST endpoints.
Unlike Start-UDDashboard, this type of server has no user interface.
