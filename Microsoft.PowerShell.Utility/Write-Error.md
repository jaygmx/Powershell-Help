...
## NOTES
\`Write-Error\` does not change the value of the \`$?\` automatic variable, therefore it does not signal a terminating error condition.
To signal a terminating error, use the $PSCmdlet.WriteError() (/dotnet/api/system.management.automation.cmdlet.writeerror)method.
