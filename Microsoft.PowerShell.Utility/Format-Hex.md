...
The \`Format-Hex\` cmdlet can help you determine the file type of a corrupted file or a file that might not have a filename extension.
You can run this cmdlet, and then read the hexadecimal output to get file information.
...
The hexadecimal output from \`Format-Hex\` shows the values of each character in the string.
...
The \`Format-Hex\` cmdlet uses the Path parameter to specify a filename in the current directory, \`File.t7f\`.
The file extension \`.t7f\` is uncommon, but the hexadecimal output \`%PDF\` shows that it is a PDF file.
In this example, the Count parameter is used to limit the output to the first 48 bytes of the file.
...
### -Encoding
Specifies the encoding of the input strings.
This only applies to \`\[string\]\` input.
The parameter has no effect on numeric types.
The output value is always \`utf8NoBOM\`.
...
Prior to PowerShell 6.2, \`Format-Hex\` would handle a Pipeline input with multiple input types by grouping all like objects together.
Now, it handles each individual object as it passes through the Pipeline and won't group objects together unless like objects are adjacent.
...
### -LiteralPath
Specifies the complete path to a file.
...
If the LiteralPath parameter includes escape characters, enclose the path in single quotation marks.
...
It is retained for script compatibility.
...
The output also states they type of bytes being processed.
If you specify the Path or LiteralPath parameter, the object contains the path of the file that contains each byte.
If you pass a string, boolean, integer, etc, it will be labeled appropriately.
