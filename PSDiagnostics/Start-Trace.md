...
For SQL database format, you must use the OutputFilePath option in the command line with the \`dsn!log\` value.
The default is binary (bin).
...
### -OutputFilePath
Path of the output log file or the DSN and log set name in a SQL database.
The default path is \`$env:systemdrive\PerfLogs\Admin\`.
...
### -SessionName
The name of the Event Trace session.
To stop a trace session you must know the session name.
